...
/* ************************************************************************* */
double NonlinearOptimizer::error() const { return state_->error; }

size_t NonlinearOptimizer::iterations() const { return state_->iterations; }

const Values& NonlinearOptimizer::values() const { return state_->values; }
...
void NonlinearOptimizer::defaultOptimize() {
  ...
  // Maybe show output
  if (params.verbosity >= NonlinearOptimizerParams::VALUES) values().print("Initial values");
  if (params.verbosity >= NonlinearOptimizerParams::ERROR)
  ...
  do {
    ...
    // User hook:
    if (params.iterationHook) params.iterationHook(iterations(), currentError, newError);

    // Maybe show output
    if (params.verbosity >= NonlinearOptimizerParams::VALUES) values().print("newValues");
    if (params.verbosity >= NonlinearOptimizerParams::ERROR)
      cout << "newError: " << newError << endl;
  } while (iterations() < params.maxIterations &&
           !checkConvergence(params.relativeErrorTol,
                             params.absoluteErrorTol,
                             params.errorTol,
                             currentError,
                             newError,
                             params.verbosity) &&
...
VectorValues NonlinearOptimizer::solve(const GaussianFactorGraph& gfg,
  ...
  if (params.isMultifrontal()) {
    ...
    if (params.ordering)
      delta =
          gfg.eliminateSequential(*params.ordering, params.getEliminationFunction())->optimize();
    else
      delta =
          gfg.eliminateSequential(params.orderingType, params.getEliminationFunction())->optimize();
  } else if (params.isIterative()) {
    // Conjugate Gradient -> needs params.iterativeParams
    if (!params.iterativeParams)
      throw std::runtime_error("NonlinearOptimizer::solve: cg parameter has to be assigned ...");

    if (auto pcg = boost::dynamic_pointer_cast<PCGSolverParameters>(params.iterativeParams)) {
      delta = PCGSolver(*pcg).optimize(gfg);
    } else if (auto spcg =
                   boost::dynamic_pointer_cast<SubgraphSolverParameters>(params.iterativeParams)) {
      if (!params.ordering) throw std::runtime_error("SubgraphSolver needs an ordering");
...
GTSAM_EXPORT bool checkConvergence(const NonlinearOptimizerParams& params,
                                   double currentError,
                                   double newError) {
  return checkConvergence(params.relativeErrorTol,
                          params.absoluteErrorTol,
                          params.errorTol,
                          currentError,
                          newError,
...
void NonlinearOptimizer::defaultOptimize(const KeyVector& keys) {
  ...
  do {
    ...
    // std::cout << "iterations " << iterations() << "| newError: " << newError << " currentError: "
    ...
    for (int i = 0; i < keys.size(); i++) {
      ...
      // std::cout << "temp_trans_norm: " << temp_trans_norm << " temp_rot_norm: " << temp_rot_norm
      // << std::endl;

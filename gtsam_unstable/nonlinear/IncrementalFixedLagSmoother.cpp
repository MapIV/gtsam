...
FixedLagSmoother::Result IncrementalFixedLagSmoother::update(const NonlinearFactorGraph& newFactors,
  ...
  // Update iSAM2
  isamResult_ = isam_.update(
      newFactors, newTheta, factorsToRemove, constrainedKeys, boost::none, additionalMarkedKeys);
  // PrintSymbolicTree(isam_, "Final Bayes Tree1:");

  // NonlinearFactorGraph dummy_f;
  // Values dummy_v;
  ...
  // // PrintSymbolicTree(isam_, "Final Bayes Tree3:");
  ...
  if (debug) {
    PrintSymbolicTree(isam_, "Final Bayes Tree4:");
    std::cout << "END" << std::endl;
  }
